...
void SetACIAStatus(unsigned char bit) {
	//if (bit == 1) WriteLog("Serial: TDRE set\n");
	ACIA_Status|=1<<bit;
}

void ResetACIAStatus(unsigned char bit) {
	//if (bit==1) WriteLog("Serial: TDRE reset\n");
	ACIA_Status&=~(1<<bit);
}
...
void Update_ACIA_IRQ() {//mm
	...
	if (SerialChannel == SerialDevice::RS423) {
		if ((TIE && ((ACIA_Status & 0x0a) ^ 0x08)) || (RIE && (ACIA_Status & 0x25))) {
			intStatus |= 1 << serial;
			SetACIAStatus(7);
		}
		else {
...
//	WriteLog("Serial: Write ACIA Tx %02X, SerialChannel = %d\n", (int)Data, SerialChannel);
	...
	intStatus&=~(1<<serial);//clear interrupt
	ResetACIAStatus(7);
...
// CSW stuff
	...
	if (SerialChannel == SerialDevice::RS423 && SerialPortEnabled)
	{
		...
		else if (EthernetPortEnabled)
		{
			...
			{
				if (RxD < 2) {
					//WriteLog("NRTS=%d\n", (ACIA_Control & 0x60) == 0x40);
#ifdef _MM_
					if ((ACIA_Control & 0x60) != 0x40)//if nrts = 0
